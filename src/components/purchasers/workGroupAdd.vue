...
<script>
  export default {
    ...
    methods:{
        //返回
        before(){
            this.$router.push({path: '/workGroupList'});
        },
        //编辑时的方法
        editMessage(id){
            let that=this;
            ...
            that.$api.branchgroupDetial_normal(params).then(response=>{
        ...
        getGroupsManageList(pageNum){
            ...
            that.$api.branchusersList_normal(params).then(response=>{
                if(response.data.msg_code===100000){
        ...
        creatGroup(){
            ...
            // }
            if(that.id==-1){
                that.$api.branchgroupCreat_normal(params).then(response=>{
                    if(response.data.msg_code===100000){
                        that.$alert('添加成功');
                        that.$router.push({path: '/workGroupList'});
                    }else{
                        that.$alert(response.data.message,'提示',{
                            confirmButtonText: '知道了',
                            callback: action => {
                ...
                params.id = that.id;
                that.$api.branchgroupUpdate_normal(params).then(response=>{
                    if(response.data.msg_code===100000){
                        that.$alert('编辑成功');
                        that.$router.push({path: '/workGroupList'});
                    }
