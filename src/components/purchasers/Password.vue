...
<script>
    export default {
        ...
        methods: {
			...
			getAuthority(){
				let that=this;
				if(that.$store.getters.authForGroup.NormalBranchChangePassword && that.$store.getters.authForGroup.NormalBranchChangePassword.transfers.length != 0){
                    that.$store.getters.authForGroup.NormalBranchChangePassword.transfers.map(item => {
