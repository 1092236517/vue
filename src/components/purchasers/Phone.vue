...
<script>
    export default {
        ...
        methods: {
			...
			getAuthority(){
				let that=this;
				if(that.$store.getters.authForGroup.NormalBranchChangePhone && that.$store.getters.authForGroup.NormalBranchChangePhone.transfers.length != 0){
					 that.$store.getters.authForGroup.NormalBranchChangePhone.transfers.map(item => {
