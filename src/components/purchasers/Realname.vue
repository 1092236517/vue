...
<script>
    export default {
        ...
        methods: {
            ...
            getAuthority(){
                let that=this;
                if(that.$store.getters.authForGroup.NormalBranchVerified&&that.$store.getters.authForGroup.NormalBranchVerified.transfers.length != 0){
                    that.$store.getters.authForGroup.NormalBranchVerified.transfers.map(item => {
