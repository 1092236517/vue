    ...
    export default {
        ...
        methods: {
            ...
            editItem(id){
                ...
                this.$router.push({path: '/demandDetial/'+id+'/'+1})
            },
            seeItem(id){
                this.isInput = false;
                this.hasEdit = false;
                //this.showThisItem(id);
                let that = this;
                that.$router.push({path: '/demandDetial/'+id+'/'+0})
            },
            //查看详情
            viewDetails(id){
                this.$router.push({path: '/branchDemandDetail/' + id})
            },
            // showThisItem(id){
            //     let that = this;
            //     let params = {
            ...
            release(){
                let that=this;
                let id='-1';
                that.$router.push({path: '/demandDetial/'+id+'/'+1})
            },
            getSelect(level,id){
                let that = this;
                let params = {
            ...
            getTabList:function(page){
                ...
                params.per_page = 10;
                that.$http.get('/api/frontend/joint_purchase/normal/branch/demand/lists', {
                    params
                }).then(function (response) {
                    if(response.data.msg_code == 100000){
