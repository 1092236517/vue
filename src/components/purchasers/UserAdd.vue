	...
	export default {
        ...
        methods: {
        	...
        	getRoles(page){
				...
				that.$api.branchroles_normal(params).then(response=>{
					if(response.data.msg_code===100000){
						//that.rolesTotal=response.data.response.total;
						that.rolesList=response.data.response.map((item)=>{
							return{
								id:item.id,
								name:item.name,
							}
			...
			rolesSure(){
				...
				this.userRoles.push(...this.workrolesSelect);
				that.DuplicateRemove(this.userRoles);
			},
            //获取组列表
            getGroups(page){
				let that=this;
				...
				that.$api.branchgroups_normal(params).then(response=>{
					if(response.data.msg_code===100000){
        	...
        	accountAdd(formName){
        		...
        		that.$refs[formName].validate(valid => { 
        			if(valid){
        				that.$api.branchuserAdd_normal(params)
        					.then(response => {
        						let result = response.data;
        						if(result.msg_code == 100000){
        							that.$message({
									...
									that.$router.push({path: '/multiAccoun'})
        						}else{
									that.$alert(result.message,'提示',{
										confirmButtonText: '知道了',
										callback: action =>{
        	...
        	accountEdit(formName){
                ...
                that.$refs[formName].validate(valid => {
                    if(valid){
                        that.$api.branchuserUpdate_normal(params, that.token)
                            .then(response => {
                                let result = response.data;
                                if(result.msg_code == 100000){
                                    that.$message({
        	...
        	getAccountInfo(id){
        		...
        		that.$api.branchaccountInfo_normal(params)
        			.then(response => {
        				let result = response.data;
        				if(result.msg_code == 100000){
