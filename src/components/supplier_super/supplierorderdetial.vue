    ...
    methods:{
                ...
                if(result.msg_code == 100000){
                    ...
                    that.orderBatchData.list = result.response.slave_orders.map(function(item){
                        let element = {
                            ...
                            send_good_spec: JSON.parse(item.send_good_spec),
                            actual_receive_spec: JSON.parse(item.actual_receive_spec),
        ...
        addThisRow(){
            this.dialogJson.spec.push({
                ...
                length: '9',
                webPrice: '--',
                supplier_total_money: '--',
                remark: ''
            });
        },
        deleteThisRow(index){
            this.dialogJson.spec.splice(index);
        },
        //确认发货弹窗
        ...
        confirmOrder(){
            ...
            let params = {
                token: that.token,
                spec: JSON.stringify(that.orderBatchDetailData.list),
        ...
        //返回按钮
        before(){
            this.$router.push({path: '/supplier_super/order_super'})
        },
        //获取批次订单进度和日志
