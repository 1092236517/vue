    		...
    		<el-form ref="form" label-position="left" :model="form" label-width="80px">
    			<el-form-item label="创建时间">
					...
					<el-col :span="9"  class="date-shotcut">
						<span @click="dateShortcut(30)">近一个月 | </span>
						<span @click="dateShortcut(90)">近三个月 | </span>
						<span @click="dateShortcut(180)">近半年 | </span>
						<span @click="dateShortcut(365)">近一年</span>
					</el-col>
    			</el-form-item>
...
<script>
    export default {
        ...
        methods: {
        	//时间选择快捷方式
        	dateShortcut(days){ //days:天数
        		let start_date = new Date();
        		let end_date = new Date();
        		start_date.setTime(start_date.getTime() - 3600 * 1000 * 24 * days);
        		this.form.start_date = start_date;
        		this.form.end_date = end_date;
        	},
        	//格式化时间
        	formatDate(date){ //Date对象格式化为'yyyy-MM-dd'
        	...
        	getList(){
        		...
        		params.start_date = that.form.start_date ? that.formatDate(that.form.start_date) : '';
        		params.end_date = that.form.end_date ? that.formatDate(that.form.end_date) : '';
