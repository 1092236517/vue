    	...
    	<div v-else-if="qualificationStauts == 'APPROVE'" v-show="statusWindow" class="status-wrapper">
   		    ...
   		    <div class="realNameBox">
				<i class="iconfont icon-Shapecopy" style="color: #0576DB;font-size: 35px;"></i>
   		    	<p>您已通过实名认证！</p>
   		    	<el-button @click="openInfoWindow" type="primary" v-show="lookBtn">去查看</el-button>
   		    </div>
    	</div>
...
<script>
    export default {
        ...
        methods: {
            ...
            getAuthority(){
				let that=this;
                if(that.$store.getters.authForGroup.Verified&&that.$store.getters.authForGroup.Verified.transfers.length!=0){
                    that.$store.getters.authForGroup.Verified.transfers.map(item=>{
                        if(item.power_transfer_name.indexOf('查看') != -1){
                            that.lookBtn = true;
                        }
                    });
                }else if(that.$store.getters.authForGroup.BranchVerified&&that.$store.getters.authForGroup.BranchVerified.transfers.length!=0){
					that.$store.getters.authForGroup.BranchVerified.transfers.map(item=>{
                        if(item.power_transfer_name.indexOf('查看') != -1){
                            that.lookBtn = true;
                        }
                    });
