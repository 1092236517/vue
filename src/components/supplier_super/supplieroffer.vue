        ...
        methods: {
            ...
            submitPrice(){
                ...
                if(that.checkArr.length==0){
                    that.$alert('该订单没有填写报价信息！');
                    return false;
                }
                for(let i=0;i<that.checkArr.length;i++){
                    if(!that.checkArr[i].price){
                        that.$alert('请输入正确的报价！');
                        return false;
                    }else{
                ...
                //     params.supplier_index_of_paid_before = that.priceArr_normal[0].price; 
                //     params.supplier_index_of_paid_after_in_tenth = that.priceArr_normal[1].price; 
                //     params.supplier_index_of_paid_after_in_ninetieth = that.priceArr_normal[2].price;  
                // }else{ 
                    url='/api/frontend/joint_purchase/super/supplier/demand_purchase_quoted_price/quota_price/';
                    params.super_quote_prices = JSON.stringify(checkArrCopy);
