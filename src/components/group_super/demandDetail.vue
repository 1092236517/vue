	...
	export default{
		...
		methods: {
			...
			getDetail(id){
				...
				that.$api.demandDetail(params,id).then(response => {
			...
			getStepsData(){
				...
				that.$api.demandProcess(params, that.id)
					.then(response => {
						let result = response.data;
						if(result.msg_code == 100000){
			...
			accountingApproveConfirm(){ //确认
			    ...
			    that.$api.customExamine(params, that.id, that.demand.currentBtn.id)
			    	.then(response => {
			    		let result = response.data;
			    		if(result.msg_code == 100000){
