...
<script>
    export default {
        ...
        created(){
            ...
            if(currentUserRoleName.groups.length != 0){
                for(let i = 0;i<currentUserRoleName.groups.length;i++){
                    this.roleName.push(currentUserRoleName.groups[i].name);
                }
            }
            if(currentUserRoleName.roles.length != 0){
                for(let i = 0;i<currentUserRoleName.roles.length;i++){
                    this.roleName.push(currentUserRoleName.roles[i].name);
                }
            }
            if(currentUserRoleName.identity == '超级管理员'){
