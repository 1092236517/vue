				  	...
				  	style="width: 100%">
				  		<el-table-column
				    		...
				    		<template slot-scope="scope">
				    			<span class="default-icon" v-show="scope.row.status == 'Using'">默</span>
				    		</template>
				  		</el-table-column>
				  		<el-table-column
				    		prop="account_name"
                        ...
                        <el-table-column
				  			...
				  			<template slot-scope="scope">
								<div v-if="scope.row.status == 'Using'" class="default-pay">默认支付</div>
				  				<button v-else type="button" class="btn table-edit-btn" @click="setDefault(scope.row.id)">设为默认支付账户</button>
...
<script>
	export default {
        ...
        methods: {
            ...
            //新增
            addPayAccount(){
                this.$router.push({name: 'groupAccountInformationAdd_super'});
			},
			//编辑
			editPayAccount(id){
				this.$router.push({name: 'groupAccountInformationEdit_super', params: {id}})
			},
			//设为默认支付
			setDefault(id){
				let that = this;
