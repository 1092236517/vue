...
export default {
    ...
    methods:{
        //返回
        before(){
            this.$router.push({path: '/group_super/workGroupList'});
        },
        //编辑时的方法
        editMessage(id){
            let that=this;
            ...
            that.$api.groupDetial(params).then(response=>{
        ...
        creatGroup(){
            ...
            // }
            if(that.id==-1){
                that.$api.groupCreat(params).then(response=>{
                    if(response.data.msg_code===100000){
                        that.$alert('添加成功');
                        that.$router.push({path: '/group_super/workGroupList'});
                    }else{
                        that.$alert(response.data.message,'提示',{
                            confirmButtonText: '知道了',
                            callback: action => {
                ...
                params.id = that.id;
                that.$api.groupUpdate(params).then(response=>{
                    if(response.data.msg_code===100000){
                        that.$alert('编辑成功');
                        that.$router.push({path: '/group_super/workGroupList'});
                    }else{
