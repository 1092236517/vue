	...
	export default {
        ...
        methods: {
            ...
            getRolesManageList(pageNum){
				...
				that.$api.mobileRolesList(params).then(response=>{
					if(response.data.msg_code===100000){
						...
						that.rolesManageList=response.data.response.data.map((item)=>{
							let workGroup = "";
							if(item.get_joint_purchase_power_group_role){
								workGroup = item.get_joint_purchase_power_group_role.get_joint_purchase_power_group.name
							}else{
								workGroup = "--";
							}
							return{
            ...
            //新增或者管理角色
            addRolesItem(id){
				this.$router.push({path: '/group_super/MobileRolesAdd/'+id})
            },
            //删除
            deleteRolesItem(id){
				let that=this;
				...
				that.$api.rolesDelete(params).then(response=>{
					if(response.data.msg_code===100000){
						that.$alert('删除成功');
            ...
            //管理角色下的用户
            manageRoleUsers(id){
				this.$router.push({path: '/group_super/multiAccountEdit_super/'+id})
            },
            //列表分页
