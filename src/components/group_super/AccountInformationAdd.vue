<template>
    <div class="add-account-wrapper">
		<h2 class="title">{{ isEdit ? '编辑付款账户' : '添加付款账户' }}</h2>
        <div class="content" v-loading="loading" element-loading-background="rgba(255, 255, 255, 0.3)">
			...
			<el-form ref="form" label-width="80px" v-model="accountForm">
				<el-form-item label="企业编号">
					...
					<el-input v-model.trim="accountForm.enterprise_code" placeholder="请输入企业编号" class="width260"></el-input>
				</el-form-item>
...
export default {
	...
	created(){
		...
		//判断编辑还是新增
		if(this.$route.name == 'groupAccountInformationEdit_super'){
			this.isEdit = true;
	...
	methods:{
		...
		addPayAccount(){
			...
			let params = {
				...
				type:'group',
				compony_code: that.accountForm.enterprise_code,
			...
			if(that.isEdit){
				that.$api.bank_accountEdit(params, that.id).then(response => {
					if(response.data.msg_code === 100000){
						...
						that.$router.push({name: 'groupAccountInformation_super'});
					}else{
		...
		async getAccountDetail(id){
			...
			this.accountForm = {
				enterprise_code: data.response.compony_code,
