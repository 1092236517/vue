    ...
    <div class="contain">
        <!-- <div class="container"> -->
            ...
            <div class="cont_3">
                    ...
                    <div class="stateCon">
                        <div class="sCon">
                            ...
                            <div class="second" v-if="count==0">
                                ...
                                <div class="conC" >
                                    <span class="timeStyle">{{day}}</span>天<span class="timeStyle">{{hour}}</span>时<span class="timeStyle">{{minute}}</span>分<span class="timeStyle">{{second}}</span>秒
                                </div>
                            </div>
                        ...
                        <div class="sCon">
                            ...
                            <div class="conC" style="width:90%;">
                                <div class="files" v-for="(item,index) in filement" :key="index">
                                    <a target="_blank" :href="item.value" :download="item.name">{{item.name}}</a>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="cont_32" v-if="state=='报价中'||state=='已流标'"><!---->
                ...
                <div class="cont_32" v-else-if="state=='发单成功'">
                    ...
                    <div class="contH" v-if='znDetial'>筑牛报价详情</div>
                    <div class="stateCon" v-if='znDetial'>
                        ...
                        <el-table :data="tabpriceList" :span-method="PriceSpanMethod" border style="width: 100%;">
                            ...
                            <el-table-column prop="unit" label="单位（元/吨）" >
                                <template slot-scope="scope">
                                    <div>
                                        <el-select
                                            v-model="scope.row.movement"
                                            placeholder="请选择"
                                            size="mini"
                                            style="width: 40%"
                                            :disabled="scope.row.tag == 'supplier'">
                                        ...
                                        <el-input
                                            ...
                                            size="mini"
                                            style="width: 50%"
                                            :disabled="scope.row.tag == 'supplier'"></el-input>
                                    </div>
                                </template>
                            </el-table-column>
                        </el-table>
        ...
        <el-dialog width="50%" title="选择付款方式" :visible.sync="paymentModeVisible" center>
            ...
            <span slot="footer" class="dialog-footer">
                ...
                <router-link to="/group_super/paymentAdd">
    ...
    methods:{
        ...
        getAuthority(){
            ...
            that.$store.getters.authForGroup.JointPurchaseOrderList.transfers.map(item=>{
                ...
                }else if(item.power_transfer_name.indexOf('筑牛报价详情')!=-1){
                    that.znDetial=true;
                }
            });
        },
            ...
            that.$http.get(that.url_prefix + '/group/demand/purchases/detail/' + id,{params} ).then(function (response) {
                if(response.data.msg_code===100000){
                    ...
                    for(let i = 0; i < lengthI; i++){
                        ...
                        let paymentModeArr = [{"description":"--"}];
                        let paymentPriceArr;
                        let id = supplierData[i].id;
                        let quotePrices = JSON.parse(supplierData[i].super_quote_prices);
                        //报价
                        if(supplierData[i].super_quote_prices &&
                            quotePrices.zn &&
                            quotePrices.zn.length > 0){
                            paymentPriceArr = JSON.parse(supplierData[i].super_quote_prices).zn;
                            //paymentPriceArr = JSON.parse(arr.total_quote_price).zn
        ...
        clickInto(id,data){
            ...
            that.tabpriceList=[];
            that.ckid=id;
            let paymentModeList = JSON.parse(data.super_quote_prices).supplier;
        ...
        submit(){
            ...
            for(let i = 0; i < that.tabpriceList.length; i++){
                if(!that.isNumber(that.tabpriceList[i].unit)){
                    that.$message({
                        message: '请填写正确的价格',
                        type: 'warning'
                    })
