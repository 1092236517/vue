	...
	export default {
        ...
        methods: {
            ...
            getGroups(page){
				...
				// 	})
				// })
				that.$api.groups(params).then(response=>{
					if(response.data.msg_code===100000){
						//that.workGroupsTotal=response.data.response.total;
        	...
        	accountAdd(formName){
        		...
        		that.$refs[formName].validate(valid => {
        			if(valid){
        				that.$api.userAdd(params).then(response => {
							let result = response.data;
							if(result.msg_code == 100000){
								that.$message({
									message: '创建成功',
								...
								that.$router.push({path: '/group_super/multiAccount_super'})
							}else{
								that.$alert(response.data.message,'提示',{
									confirmButtonText: '知道了',
									callback: action => {
        	...
        	accountEdit(formName){
                ...
                that.$refs[formName].validate(valid => {
                    if(valid){
                        that.$api.userUpdate(params, that.token)
                            .then(response => {
                                let result = response.data;
                                if(result.msg_code == 100000){
                                    that.$message({
