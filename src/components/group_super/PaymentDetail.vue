<template>
	<div class="payment-detail"
		...
		<div class="item-div">
			<span class="title">付款单状态：</span>
			<span class="desc" :class="paymentStatusStyle">{{ status | statusToChinese}}</span>
		</div>
		<div class="item-div">
			<span class="title">付款状态描述：</span>
			<span class="desc">{{ bank_payment_desc }}</span>
		</div>
		<div class="item-div">
			<span class="title">付款时间：</span>
			<span class="desc">{{ pay_time }}</span>
		</div>
		...
		<div class="button-div">
			...
			<el-button class="confirm-btn" type="primary" v-if="wait_to_handle" @click="confirmedToPay">确定</el-button>
		</div>
	</div>
</template>
	...
	export default{
		...
		computed: {
			paymentStatusStyle(){ //支付状态样式
				...
				switch(this.status) {
					case 'WAIT_PAY':
						styleObj['payment-wait'] = true;
						break;
					case 'WAIT_BANK_PAY':
						styleObj['payment-wait'] = true;
						break;
					case 'PAY_ALREADY':
						styleObj['payment-success'] = true;
						break;
					case 'PAY_FAIL':
						styleObj['payment-error'] = true;
						break;
					default:
						styleObj['payment-default'] = true;
						break;
				}
				return styleObj
			},
			wait_to_handle(){
				return this.status == 'WAIT_PAY'
			}
		},
		created(){
		...
		methods: {
			...
			getDetail(id){
				...
				that.loading = true;
				that.$api.groupPayToZhuniuDetail(params, id)
					.then(response => {
						let result = response.data;
						let paymentData = result.response;
						that.loading = false;
			...
			confirmedToPay(){
			    ...
			    let params ={
			        token: that.token,
			        id: that.id,
			        slave_order_id: that.slave_order_id
			    }
			    that.$confirm('是否确认付款？','提示',{
			        ...
			        that.$api.grouptConfirmToPay(params).then(response => {
			            let result = response.data;
			            if(result.msg_code == 100000){
			                that.$router.push({name: 'groupPayResultApproved_super'})
			            }else{
			                that.$alert(result.message, '提示', {
			                    confirmButtonText: '知道了',
			                    callback: action => {
			...
			//返回到支付列表页
			goToPaymentList(){
				this.$router.push({name: 'groupPaymentInformationList_super'})
			}
		}
	}
</script>
