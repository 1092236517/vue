...
export default {
    ...
    methods:{
        //返回
        before(){
            this.$router.push({path: '/group_super/MobileRoleManagement'})
        },
        //修改——信息回填
        ...
        roleCategory(){
            ...
            that.$api.roleCategory(params).then(response=>{
                if(response.data.msg_code === 100000){
                    that.rolesType=response.data.response.data;
                    console.log(that.rolesType)
                }else{
                    that.$alert(response.data.message,'提示',{
                        confirmButtonText: '知道了',
                        callback: action => {
        ...
        submit(){
            ...
            if(that.id==-1){
                ...
                that.$api.mobileroleCreat(params).then(response=>{
                    if(response.data.msg_code===100000){
                        that.$alert('添加成功');
                        that.$router.push({path: '/group_super/MobileRoleManagement'})
                    }else{
                        that.$alert(response.data.message,'提示',{
                            confirmButtonText: '知道了',
                            callback: action => {
                ...
                that.$api.mobileRoleUpdate(params).then(response=>{
                    if(response.data.msg_code===100000){
                        that.$alert('修改成功');
                        that.$router.push({path: '/group_super/MobileRoleManagement'})
                    }else{
