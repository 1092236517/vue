...
export default {
    ...
    methods:{
        //返回
        before(){
            this.$router.push({path: '/purchasers_super/MobileRoleManagement'})
        },
        //修改——信息回填
        getRolesManageList(id){
            ...
            that.$api.branchrolesList(params).then(response=>{
                if(response.data.msg_code===100000){
                    ...
                    that.$refs.tree.setCheckedKeys(response.data.response.data[0].get_j_p_mobile_power_role_transfers.map((item)=>{
        ...
        roleCategory(){
            ...
            that.$api.branchroleCategory(params).then(response=>{
                if(response.data.msg_code === 100000){
        ...
        permissions(){
            ...
            that.$api.mobilebranchpermissionsList(params).then(response=>{
                if(response.data.msg_code === 100000){
        ...
        submit(){
            ...
            if(that.id==-1){
                ...
                that.$api.mobilebranchroleCreat(params).then(response=>{
                    if(response.data.msg_code===100000){
                        that.$alert('添加成功');
                        that.$router.push({path: '/purchasers_super/MobileRoleManagement'})
                    }else{
                        that.$alert(response.data.message,'提示',{
                            confirmButtonText: '知道了',
                            callback: action => {
                ...
                //     }));
                // }
                that.$api.mobilebranchroleUpdate(params).then(response=>{
                    if(response.data.msg_code===100000){
                        that.$alert('修改成功');
                        that.$router.push({path: '/purchasers_super/MobileRoleManagement'})
                    }else{
