...
<script>
    export default {
        ...
        methods: {
            ...
            getAuthority(){
				let that=this;
                if(that.$store.getters.authForGroup.BranchMultiAccount && that.$store.getters.authForGroup.BranchMultiAccount.transfers.length > 0){
                    that.$store.getters.authForGroup.BranchMultiAccount.transfers.map(item=>{
			...
			//新增
        	addUsers(){
        	    this.$router.push({path: '/purchasers_super/userAdd'})
        	},
        	//载入账户列表
        	getList(options){
				...
				//let url = that.url_prefix + '/member/account/list'
        	...
        	getType(){
        		...
        		that.$http.get('/api/frontend/member/roles',{params}).then(function(res){ //接口statusCode:500
		    ...
		    editAccount: function(id){
				...
				// })
				this.$router.push({path: '/purchasers_super/userEdit/' + id})
		    },
		    //提交编辑函数
		    submitEdit: function(){
		    	let that = this;
