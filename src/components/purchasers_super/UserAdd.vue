	...
	export default {
        ...
        methods: {
        	...
        	getRoles(page){
				...
				that.$api.branchroles(params).then(response=>{
            ...
            getGroups(page){
				...
				that.$api.branchgroups(params).then(response=>{
					if(response.data.msg_code===100000){
        	...
        	accountEdit(formName){
                ...
                that.$refs[formName].validate(valid => {
                    if(valid){
                        that.$api.branchuserUpdate(params, that.token)
                            .then(response => {
                                let result = response.data;
                                if(result.msg_code == 100000){
                                    that.$message({
        	...
        	getAccountInfo(id){
        		...
        		that.$api.branchaccountInfo(params)
        			.then(response => {
        				let result = response.data;
        				if(result.msg_code == 100000){
