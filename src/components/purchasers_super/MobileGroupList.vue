	...
	export default {
        ...
        methods: {
            ...
            getGroupList(pageNum){
                ...
                that.$api.branchgroupList(params).then(response=>{
            ...
            //新增或编辑
            addRolesItem(id){
                this.$router.push({path: '/purchasers_super/MobileGroupAdd/'+id})
            },
            //删除
            deleteRolesItem(id){
				let that = this;
				...
				that.$api.branchgroupDelete(params).then(response=>{
					if(response.data.msg_code==100000){
						that.$alert('删除成功');
