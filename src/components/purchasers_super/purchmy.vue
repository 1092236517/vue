    ...
    export default {
        ...
        methods: {
            ...
            getMsgList(status,curPage){ //status的值: ''(全部),0(未读),1(已读);curPage当前页
        		...
        		that.$http.get('/api/frontend/joint_purchase/super/branch/message/lists',{params})
        			.then(function(response){
        				let result = response.data;
        				if(result.msg_code == 100000){
            ...
            signRead(item){
                ...
                if(item.is_read == 0){
                    params.id = item.id;
                    params.token = that.token;
                    that.$http.post('/api/frontend/joint_purchase/super/branch/message/read',params)
                        .then(function(response){
                            let result = response.data;
                            if(result.msg_code == 100000){
                                item.is_read = 1;
