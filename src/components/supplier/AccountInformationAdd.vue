...
export default {
	...
	created(){
		...
		//判断编辑还是新增
		if(this.$route.name == 'accountinformationEdit_supplier'){
			this.isEdit = true;
	...
	methods:{
		...
		addPayAccount(){
			...
			if(that.isEdit){
				that.$api.bank_accountEdit_supplier(params, that.id).then(response => {
					if(response.data.msg_code === 100000){
						that.$message({
						    message: '修改成功',
						    type: 'success'
						});
						that.$router.push({name: 'supplierAccountInformation'});
					}else{
		...
		async getAccountDetail(id){
			...
			that.loading = true;
			let params = {token: that.token};
			let {data} = await that.$api.bank_accountDetail_supplier(params, id);
