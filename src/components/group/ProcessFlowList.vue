	...
	export default {
		...
		methods: {
			...
			getProcessFlowList(){
				...
				that.$api.processFlowList_normal(params)
					.then(response => {
			...
			//添加流程
			addProcessFlow(){
				this.$router.push({path: '/group/createProcessFlow'})
			},
			//产看详情
			viewDetail(id){
				this.$router.push({path: '/group/editProcessFlow/' + id})
			},
            //删除流程
            deleteFlows(){
            	let that = this;
            	...
            	that.$api.processFlowDelete_normal(params)
            		.then(response => {
                        let result = response.data;
                        if(result.msg_code == 100000){
                        	that.$message({
