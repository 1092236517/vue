...
export default {
    ...
    methods:{
        //返回
        before(){
            this.$router.push({path: '/group/RolesManage'})
        },
        //修改——信息回填
        ...
        roleCategory(){
            ...
            that.$api.roleCategory_normal(params).then(response=>{
                if(response.data.msg_code === 100000){
        ...
        submit(){
            ...
            if(that.id==-1){
                ...
                that.$api.roleCreat_normal(params).then(response=>{
                    if(response.data.msg_code===100000){
                        that.$alert('添加成功');
                        that.$router.push({path: '/group/RolesManage'})
                    }else{
                        that.$alert(response.data.message,'提示',{
                            confirmButtonText: '知道了',
                            callback: action => {
                ...
                //     }));
                // }
                that.$api.roleUpdate_normal(params).then(response=>{
                    if(response.data.msg_code===100000){
                        that.$alert('修改成功');
                        that.$router.push({path: '/group/RolesManage'})
                    }else{
