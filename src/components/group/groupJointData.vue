        ...
        <el-table
            ...
            <el-table-column  label="操作" width="180">
                <template slot-scope="scope">
                    <router-link :to="'/group/jointdatadetail/' + scope.row.id"><el-button type="primary" v-show="lookButton">查看详情</el-button></router-link>
                </template>
            </el-table-column >
        </el-table>
    </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            getAuthority(){
                let that=this;
                that.$store.getters.authForGroup.NormalJointPurchaseDataSummary.transfers.map(item=>{
            ...
            goToDetail(id){
                this.$router.push({path: '/group/jointdatadetail/'+id})
            }
        },
        mounted(){
        	// 设置下token
