...
<script>
    export default {
        ...
        methods: {
            ...
            getAuthority(){
				let that=this;
                if(that.$store.getters.authForGroup.NormalMultipleAccountSettings.transfers.length!=0){
                    that.$store.getters.authForGroup.NormalMultipleAccountSettings.transfers.map(item=>{
        	...
        	getUsersList(page){
        		...
        		that.$api.usersListNew_normal(params)
        			.then(response => {
        	...
        	//新增
        	addUsers(){
        	    this.$router.push({path: '/group/userAdd'})
        	},
        	//删除
        	deleteRolesItem(){
        	...
        	//编辑
        	editUser(id){
        		this.$router.push({path: '/group/userEdit/' + id})
        	},
		    //账户禁用/启用
