...
export default {
    ...
    methods:{
        //返回
        before(){
            this.$router.push({path: '/group/workGroupList'});
        },
        //编辑时的方法
        editMessage(id){
            let that=this;
            ...
            that.$api.groupDetial_normal(params).then(response=>{
                if(response.data.msg_code===100000){
                    ...
                    that.workrolesList=response.data.response.groupRoles.map(item=>{
                        return{
                            name:item.get_role.name,
                            id:item.get_role.id,
                        }
                    });
                    that.$refs.tree.setCheckedKeys(response.data.response.transfers[0].get_group_transfers.map((item)=>{
        ...
        getRolesManageList(pageNum){
            ...
            that.$api.rolesList_normal(params).then(response=>{
                if(response.data.msg_code===100000){
        ...
        permissions(){
            ...
            //     type:'GROUP',
            // }
            // that.$api.permissionsList(params).then(response=>{
        ...
        getGroupsManageList(pageNum){
            ...
            that.$api.usersList_normal(params).then(response=>{
                if(response.data.msg_code===100000){
        ...
        creatGroup(){
            ...
            console.log(that.workrolesList);
            let params={
                token:that.token,
                name:that.groupName,
            ...
            // }
            if(that.id==-1){
                that.$api.groupCreat_normal(params).then(response=>{
                    if(response.data.msg_code===100000){
                        that.$alert('添加成功');
                        that.$router.push({path: '/group/workGroupList'});
                    }else{
                        that.$alert(response.data.message,'提示',{
                            confirmButtonText: '知道了',
                            callback: action => {
                ...
                params.id = that.id;
                that.$api.groupUpdate_normal(params).then(response=>{
                    if(response.data.msg_code===100000){
                        that.$alert('编辑成功');
                        that.$router.push({path: '/group/workGroupList'});
                    }else{
