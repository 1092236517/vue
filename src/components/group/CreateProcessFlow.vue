	...
	export default {
		...
		methods: {
			...
			getApplyList(){
				...
				that.$api.processFlowApplyList_normal(params)
					.then(response => {
			...
			getProcessFlowDetail(id){
				...
				that.$api.processFlowDetail_normal(params, id)
					.then(response => {
						let result = response.data;
						if(result.msg_code == 100000){
			...
			createProcessFlow(){
				...
				that.$api.processFlowAdd_normal(params)
					.then(response => {
						let result = response.data;
						if(result.msg_code == 100000){
							that.$message({
							...
							that.$router.push('/group/processFlowList');
						}else{
							that.$message({
