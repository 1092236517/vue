...
<script>
    export default {
        ...
        methods: {
            ...
            getAuthority(){
                let that=this;
                if(that.$store.getters.authForGroup.NormalVerified&&that.$store.getters.authForGroup.NormalVerified.transfers.length!=0){
                    that.$store.getters.authForGroup.NormalVerified.transfers.map(item=>{
