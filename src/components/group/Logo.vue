...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getAuthority(){
                let that=this;
                if(that.$store.getters.authForGroup.NormalLOGO && that.$store.getters.authForGroup.NormalLOGO.transfers.length != 0){
                    that.$store.getters.authForGroup.NormalLOGO.transfers.map(item => {
