    ...
    <div>
        ...
        <el-table
            ...
            <el-table-column  label="操作" width="230">
                <template slot-scope="scope">
                    <div class="nomalBtn1 tabelBtn" v-if='scope.row.status == "履约中"&&closure' @click="seeItem(scope.row.id)">申请关闭</div>
...
<script>
    export default {
        ...
        methods: {
            ...
            order(id){
                let that = this;
                that.$router.push({path: '/group/order/'+id})
            },
            seeItem(id){
                let that = this;
                that.$prompt('', '请输入关闭原因', {
                    ...
                    that.$http.get('/api/frontend/joint_purchase/normal/group/order/apply_close_order/'+id, {
                        params
            ...
            getTabList(page){
                ...
                that.$http.get('/api/frontend/joint_purchase/normal/group/order/lists', {
                    params
                }).then(function (response) {
                    if(response.data.msg_code == 100000){
