...
export default {
    ...
    methods:{
        ...
        getAuthority(){
            let that=this;
            if(that.$store.getters.authForGroup.NormalSlaveOrderReport && that.$store.getters.authForGroup.NormalSlaveOrderReport.transfers.length!=0){
                that.$store.getters.authForGroup.NormalSlaveOrderReport.transfers.map(item=>{
        ...
        onSubmit(pageNum){
            ...
            that.$api.orderReportSum_normal(params).then(response=>{
                that.sumNum = response.data.response.sum;
            })
            that.$api.orderReportCount_normal(params).then(response=>{
                 that.sumPrice = response.data.response.count;
            })
            that.$api.orderReportList_normal(params).then(response=>{
