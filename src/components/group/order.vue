<template>
    <div>
        ...
        <el-table
            ...
            border
            style="width: 100%">
            <el-table-column  prop="batch_number" label="批次号"><!--type="index" :index="indexMethod"-->
            </el-table-column>
...
<!--                     <div class="nomalBtn1 tabelBtn" @click="seeItem1(scope.row, 2)" v-if="customCheckBtn && scope.row.currentBtn">审核批次</div>
                    ...
                    <div class="nomalBtn1 tabelBtn" @click="seeItem4(scope.row)" v-if="scope.row.status == 'WAIT_GROUP_PAY' && PaymentBtn && groupCanPay(scope.row)">支付批次</div>
                </template>
            </el-table-column >
        </el-table>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getAuthority(){
                let that=this;
                if(that.$store.getters.authForGroup.NormalOrderList && that.$store.getters.authForGroup.NormalOrderList.transfers.length > 0){
                    that.$store.getters.authForGroup.NormalOrderList.transfers.map(item=>{
            ...
            customCheck(params, orderId, recordId){
                let that = this;
                that.$api.customExamineForBatchOrder_normal(params, orderId, recordId)
                    .then(response => {
                        let result = response.data;
                        if(result.msg_code == 100000){
                            that.dialogVisible = false;
            ...
            departmentSignCheck(params, id){
                let that = this;
                that.$api.departmentSignCheckForBatchOrder_normal(params, id)
                    .then(response => {
                        let result = response.data;
                        if(result.msg_code == 100000){
                            that.dialogVisible = false;
            ...
            signCheck(params, id){
                let that = this;
                that.$api.signCheckForBatchOrder_normal(params, id)
                    .then(response => {
                        let result = response.data;
                        if(result.msg_code == 100000){
                            that.dialogVisible = false;
            ...
            getStepsData(slave_order_id){
                ...
                that.$api.groupBatchOrderProcess_normal(params)
                    .then(response => {
                        let result = response.data;
                        if(result.msg_code == 100000){
            ...
            getStepsLog(id){
                ...
                that.$api.groupBatchOrderLogList_normal(params, id)
                    .then(response => {
                        let result = response.data;
                        if(result.msg_code == 100000){
            ...
            //财务部能否支付判断
            groupCanPay(item){
                let group_can_pay_time = +new Date(item.group_can_pay_time);
                let pay_type = item.paid_type;
                if(pay_type == 'PAID_BEFORE'){
                    return true
                }
                if(pay_type != 'PAID_BEFORE' && group_can_pay_time < +new Date()){
                    return true
                }
                return false
            }
