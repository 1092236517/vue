...
<script>
    export default{
        ...
        methods: {
            ...
            getAuthority(){
                let that=this;
                if(that.$store.getters.authForGroup.NormalSecondPaymentManagement.transfers.length!=0){
                    that.$store.getters.authForGroup.NormalSecondPaymentManagement.transfers.map(item=>{
            ...
            //跳转到添加支付方式页面
            addPaymentMode(){
                this.$router.push('/group/paymentadd');
            },
            //删除当前支付方式
            deleteMode(item){
                let that = this;
