...
export default {
    ...
    methods:{
        //返回
        before(){
            this.$router.push({path: '/purchasers_super/RolesManage'})
        },
        //修改——信息回填
        ...
        submit(){
            ...
            if(that.id==-1){
                ...
                that.$api.branchroleCreat(params).then(response=>{
                    if(response.data.msg_code===100000){
                        that.$alert('添加成功');
                        that.$router.push({path: '/purchasers_super/RolesManage'})
                    }else{
                        that.$alert(response.data.message,'提示',{
                            confirmButtonText: '知道了',
                            callback: action => {
                ...
                //     }));
                // }
                that.$api.branchroleUpdate(params).then(response=>{
                    if(response.data.msg_code===100000){
                        that.$alert('修改成功');
                        that.$router.push({path: '/purchasers_super/RolesManage'})
                    }else{
