...
<script>
  export default {
    ...
    methods:{
        //返回
        before(){
            this.$router.push({path: '/purchasers_super/workGroupList'});
        },
        //编辑时的方法
        editMessage(id){
            let that=this;
            ...
            that.$api.branchgroupDetial(params).then(response=>{
                if(response.data.msg_code===100000){
                    ...
                    that.$refs.tree.setCheckedKeys(response.data.response.transfers[0].get_joint_purchase_power_group_transfers.map((item)=>{
                    ...
                    // that.checkedItem=response.data.response.transfers[0].get_joint_purchase_power_group_transfers.map((item)=>{
        ...
        getRolesManageList(pageNum){
            ...
            that.$api.branchrolesList(params).then(response=>{
                if(response.data.msg_code===100000){
        ...
        getGroupsManageList(pageNum){
            ...
            that.$api.branchusersList(params).then(response=>{
                if(response.data.msg_code===100000){
        ...
        creatGroup(){
            ...
            // }
            if(that.id==-1){
                that.$api.branchgroupCreat(params).then(response=>{
                    if(response.data.msg_code===100000){
                        that.$alert('添加成功');
                        that.$router.push({path: '/purchasers_super/workGroupList'});
                    }else{
                        that.$alert(response.data.message,'提示',{
                            confirmButtonText: '知道了',
                            callback: action => {
                ...
                params.id = that.id;
                that.$api.branchgroupUpdate(params).then(response=>{
                    if(response.data.msg_code===100000){
                        that.$alert('编辑成功');
                        that.$router.push({path: '/purchasers_super/workGroupList'});
                    }
